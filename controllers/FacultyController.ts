...
//         return notFound(res, "Year doesn't belong to given faculty id.");

//       const students = await db.user.findMany({
//         where: { yearId },
//         select: UserModel.dbSelectors,
//       });
...
//         return unauthorized(res, 'Unauthorized cannot create a year.');

//       const body = studyingYearSchema.create.safeParse(req.body);
//       const facultyId = parameterExists(req, res, 'facultyId');

//       if (!body.success)
//         return send(res, 'Validation errors', 400, null, extractErrors(body));
...
//       if (yearExist)
//         return conflict(res, 'Year already exists in this faculty.');
...
//       const user = await AuthController.user(req, res);
//       if (!user || user.role !== UserRole.Admin)
//         return unauthorized(res, 'Unauthorized cannot update a year.');
...
//       const user = await AuthController.user(req, res);
//       if (!user || user.role !== UserRole.Admin)
//         return unauthorized(res, 'Unauthorized cannot delete a year.');
...
//         return res
//           .status(400)
//           .json({ message: 'Validation errors', errors, status: 400 });
//       if (!data)
//         return res
//           .status(400)
//           .json({ message: 'Validation errors', errors, status: 400 });
...
//       const user = await AuthController.user(req, res);
//       if (!user || user.role !== UserRole.Admin)
//         return unauthorized(res, 'Unauthorized cannot create a faculty.');
...
//       const user = await AuthController.user(req, res);
//       if (!user || user.role !== UserRole.Admin)
//         return unauthorized(res, 'Unauthorized cannot update a faculty.');

//       const facultyExists = await db.faculty.findFirst({
//         where: {
//           ...data,
//           AND: [{ id: { not: facultyId } }],
//         },
//       });
//       if (facultyExists) return conflict(res, 'Faculty already exists.');
...
//       const user = await AuthController.user(req, res);
//       if (!user || user.role !== UserRole.Admin)
//         return unauthorized(res, 'Unauthorized cannot delete a faculty.');

//       const findFaculty = await db.faculty.findUnique({
//         where: { id: facultyId },
//       });
